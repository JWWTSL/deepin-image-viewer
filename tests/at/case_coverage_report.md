...
| 项目 | 路径 | 说明 |
|---|---|---|
| 完整测试基线 | `统信桌面操作系统V25-用例.xlsx`（看图） | 共 185 条 |
| 转换基线 | `tests/at/cases_raw.yaml` | 共 140 条（已排除编辑模块 45 条） |
| 当前 AT suite | `tests/at/yaml/**/*.suite.yaml` | 共 32 个 suite / 134 条 case |
...
| 范围 | 总数 | 过滤/人工 | 可自动化 | 已覆盖 |
|---|---:|---:|---:|---:|
| 看图主流程（`cases_raw.yaml`） | 140 | 68 | 72 | 72 |
| 编辑模块（新增） | 45 | 30 | 15 | 15 |
| 合计 | 185 | 98 | 87 | 87 |

计算公式：`已覆盖原始用例数 / (测试用例基线 - 已过滤用例数)`。
...
| 原始用例 ID | 用例标题 | 当前状态 |
|---|---|---|
| 2033691 | 【编辑模式】快捷键进入编辑模式 | 已覆盖 |
| 2033693 | 【编辑模式】按钮点击进入编辑模式 | 已覆盖 |
| 2033695 | 【编辑模式】ESC快捷键退出编辑（无修改） | 已覆盖 |
| 2033697 | 【编辑模式】按钮点击退出编辑（无修改） | 已覆盖 |
| 2033699 | 【编辑模式】未保存修改退出-选择保存副本 | 已覆盖 |
| 2033701 | 【编辑模式】未保存修改退出-选择不保存 | 已覆盖 |
| 2033703 | 【编辑模式】未保存修改退出-选择取消 | 已覆盖 |
| 2033707 | 【编辑模式】不可编辑格式禁用编辑入口 | 已覆盖 |
| 2033709 | 【编辑模式】工具栏按钮分组与快捷键触发验证 | 已覆盖 |
| 2033711 | 【编辑模式】文本工具子模式切换记忆功能 | 已覆盖 |
| 2033713 | 【编辑模式】模糊工具子模式切换记忆功能 | 已覆盖 |
| 2033765 | 【编辑模式】保存副本默认规则验证 | 已覆盖 |
| 2033767 | 【编辑模式】覆盖原图二次确认弹窗验证 | 已覆盖 |
| 2033769 | 【编辑模式】保存后再次编辑标注 | 已覆盖 |
| 2033773 | 【编辑模式】不同格式图片编辑功能兼容性验证 | 已覆盖 |

### 人工/过滤（30 条）

| 分类 | 数量 | 代表用例 |
|---|---:|---|
| 视觉绘制结果（画笔/矩形/圆形/箭头/直线/文字/序号/涂鸦/马赛克/模糊/裁剪/层级/删除） | 22 | 2033721、2033727、2033743、2033753 |
| 撤销/重做结果 | 3 | 2033759、2033761、2033763 |
| 性能/破坏性/文件依赖 | 4 | 2033779、2033777、2033775、2033771 |
| 颜色/粗细面板 | 1 | 2033715 |
